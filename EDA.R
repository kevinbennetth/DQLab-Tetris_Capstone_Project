...
## There is inconsistent amount of data present. Therefore I decided to select just the top 5 popular and well-known e-commerce platforms in Indonesia.
shops <- c('tokopedia','shopee','lazada','blibli','bukalapak')
...
df2017 <- df2017_binded %>% group_by(Name) %>%
  summarise(across(c(Twitter,Instagram,Facebook), max), Traffic=mean(Traffic))
...
df2017$Year <- rep(2017,5)
df2018$Year <- rep(2018,5)
df2019$Year <- rep(2019,5)
...
r_2021 <- q4_2021 %>%
  ...
  filter(tolower(Name) %in% shops)
r_2021[2:3] <- as_tibble(apply(r_2021[2:3], 2, function(x) as.numeric(gsub(pattern="#",replacement="",x,fixed=TRUE))))

### Adding a column to represent year
r_2017$Year <- rep(2017,5)
r_2018$Year <- rep(2018,5)
r_2019$Year <- rep(2019,5)
r_2020$Year <- rep(2020,5)
r_2021$Year <- rep(2021,5)
...
fig_web <- plot_ly(df_p_prepped, x=~Year, y=~Traffic, split=~Name, color=~Name,
...
fig1 <- plot_ly(df_p_prepped, x=~Year, y=~Twitter, split=~Name, legendgroup=~Name, color=~Name,
                type="scatter", mode="lines+markers") %>%
  layout(yaxis=list(title="Twitter"))

### Plotting Instagram popularity
fig2 <- plot_ly(df_p_prepped, x=~Year, y=~Instagram, split=~Name, legendgroup=~Name, color=~Name,
                type="scatter", mode="lines+markers", showlegend=FALSE) %>%
  layout(yaxis=list(title="Instagram"))

### Plotting Facebook popularity
fig3 <- plot_ly(df_p_prepped, x=~Year, y=~Facebook, split=~Name, legendgroup=~Name, color=~Name,
                type="scatter", mode="lines+markers", showlegend=FALSE) %>%
...
# CONCLUSION
## With the limitations of data availability, I was only able to acquire a semi-raw data (was collected by another party) of average web visits/traffic including store rankings along with social media popularity (measured by followers).
